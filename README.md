...
Bellows is a no-compromises drop-in Fabric mod designed to apply targeted optimizations for Minecraft commands-- especially in environments with many large datapacks. Do to the targeted nature of Bellow's optimizations, performance gains are somewhat variable. Most large datapacks see a 50-60% reduction in tick time using Bellows. Certian packs like Animated Java can see a 70-90% reduction in tick time.
...
* You are in a controlled enviroment, ie. your own server which is running Bellows
